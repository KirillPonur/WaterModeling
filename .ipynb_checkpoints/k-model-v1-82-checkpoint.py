...
k=arange(KT[0]*10**(-10),KT[-1]+500,0.01)
...
for i in range(len(t)):
    model_t.append(main.model(k,phi,t[i]) )
...
for i in range(len(t)):
    ...
    contourf(z,100,cmap=cm.winter, vmin=-0.4, vmax=0.4)
    # colorbar()
    # ylabel(r'Y, м',fontsize=16)
    # xlabel(r'X, м',fontsize=16)
    savefig(path.abspath('..'+'\\water\\anim\\'+'water'+str(i)+'.png'),
            pdi=10**3,bbox_inches='tight')
# show()